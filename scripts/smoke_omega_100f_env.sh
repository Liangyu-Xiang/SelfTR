#!/usr/bin/env bash
# Verify that a VGGT-Omega environment can load the Omega checkpoint and run
# a 100-frame 7Scenes forward pass.
set -euo pipefail
...
SAMPLING_STRIDE=${SAMPLING_STRIDE:-3}
...
OUTPUT_DIR=${OUTPUT_DIR:-outputs/smoke_omega_100f_env}
...
def main() -> int:
    ...
    return 0 if all(result.get("success") for result in summary["results"]) else 1
